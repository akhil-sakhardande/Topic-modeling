...
{
excel <- read.xlsx("FGD- Paddy sheets.xlsx", sheetIndex = each, rowIndex = 12:54, colIndex = 2:3)
class(excel)
head(excel,3)
dim(excel)
...
excel_res1$Region <- as.character(Sheets[each])
...
write.table(excel_res1,file = "/home/asakhardhande19/rallis_files/final_paddy_topics.csv", sep = ",", row.names=FALSE, append=TRUE, col.names = ifelse(each %in% 1, TRUE, FALSE))
}
