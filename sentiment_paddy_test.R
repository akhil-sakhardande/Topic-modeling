...
# creating a looping construct to loop through each sheet
  ...
  excel <- read.xlsx("FGD- Paddy sheets.xlsx", sheetIndex = 8, rowIndex = 2:11, colIndex = 2:3)
  class(excel)
  head(excel,3)
  names(excel)
  dim(excel)
  
  # Collating the answer data together
  ...
  out_pol2$Polarity.Score1 <- pol2$group.ave.polarity
  excel_res
  ...
  out_pol2$Region <- as.character(Sheets[8])
  ...
  write.table(out_pol2,file = "/home/asakhardhande19/rallis_files/test_paddy_opinions.csv", sep = ",", row.names=FALSE, append=TRUE, col.names = TRUE)
